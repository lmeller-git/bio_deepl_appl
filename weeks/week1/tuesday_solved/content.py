...
# DNA to RNA conversion dictionary
...
# Function to convert DNA sequence to amino acid sequence
...
def dna_to_amino_acid(dna_sequence):
    ...
    for i in range(0, len(rna_sequence), 3):
        ...
        if codon in rna_to_aminoacid:

            amino_acid = rna_to_aminoacid[codon]
...
class Language:
    ...
    def encode(self, sentence):
        ...
        sentence = dna_to_amino_acid(sentence)
    ...
    def encode_dataset(self, dataset, dataset2 = None):
        ...
        if dataset2 is not None :
            ...
            encoded_negatives = [{'sample':self.sample_sentence(sentence),'label':torch.Tensor([0])} for sentence in dataset2]
...
for cur_seq in seq_data_human:
    ...
    #cur_seq = cur_seq[1:] # frameshift

    cur_seq = dna_to_amino_acid(cur_seq)
...
train_set_yeast_human_encoded = dna_lang.encode_dataset(train_set_yeast, train_set_human)

val_set_yeast_human_encoded = dna_lang.encode_dataset(val_set_yeast, val_set_human)

test_set_yeast_human_encoded = dna_lang.encode_dataset(test_set_yeast, test_set_human)
...
train_loader_yeast_human = get_dataloader(train_set_yeast_human_encoded, batch_size)

val_loader_yeast_human = get_dataloader(val_set_yeast_human_encoded, batch_size)

test_loader_yeast_human = get_dataloader(test_set_yeast_human_encoded, 1)
...
test_loader_yeast = get_dataloader(test_set_yeast_encoded, 1)
...
test_sampler = enumerate(test_loader_yeast_human)
...
1.:input_param * output_param + output

2.:yeah relu

3.: some species are more closely related ie human and mouse and thus harder to classify

4.: works

5.: accuracy degrades stringly
