...
class SequenceData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.zeros(
            (1, len(sequence_encoding), 20)
        )  # will be 1 where we have a measurement
        ...
        return {
            "sequence": sequence_encoding[None, :, :].float(),
            "mask": mask,
            "labels": target,
        }
...
def get_df():
    ...
    for batch in dataloader_val:
        ...
        preds.append(
            prediction[mask == 1].flatten().detach().numpy()
        )  # flatten to create one dimensional vector from 2D sequence

        all_y.append(
            target[mask == 1].flatten().detach().numpy()
        )  # flatten to create one dimensional vector from 2D sequence
